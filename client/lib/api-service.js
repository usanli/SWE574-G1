...
export const postService = {
  ...
  async getPosts(page = 1, limit = 9, sortBy = 'recent', filter = 'all', searchQuery = '') {
    ...
    const posts = response.map(mystery => ({
      ...
      downvotes: mystery.votes?.filter(v => v.type === 'downvote').length || 0,
      commentCount: mystery.comments?.length || 0,
      status: mystery.solved_by ? 'solved' : 'unsolved',
      createdAt: mystery.created_at,
      author: {
        name: mystery.anonymous ? 'Anonymous' : mystery.author?.username || 'Unknown',
        avatar: mystery.anonymous ? '/placeholder.svg' : mystery.author?.profile_picture_url || '/placeholder.svg',
      },
    }));
